...
function Weather(props) {
  ...
  const setNewAutocompleteVal = (newVal) => {
    ...
    if (!newVal) {
      ...
      props.history.push(
        `/weather/${newVal.Key}/?search=${newVal.LocalizedName}`
      );
      props.dispatchCurrentWeatherInfo({
        name: `${newVal.LocalizedName}${
          newVal.Country ? ", " + newVal.Country.LocalizedName : ""
        }`,
        key: newVal.Key,
      });
    }
  ...
  const onMountedData = async () => {
    ...
    if (props.match.params.locationId) {
      ...
      const itemInAutocompleteOptions = cities.find(
        (option) => option.Key === props.match.params.locationId
      );
  ...
  useDebounce(autocompleteInput, 1000, async () => {
    if (autocompleteInput)
      setAutocompleteItems(await fetchAutocompleteCities(autocompleteInput));
  });
  // Render
